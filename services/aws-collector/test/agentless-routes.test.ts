...
test("malformed and dangerous requests are refused before anything is claimed", async () => {
  await withServer(async (baseUrl, secret) => {
    const cases: readonly (readonly [string, unknown])[] = [
      ["not an object", "nope"],
      ["missing settings", { ...REQUEST, settings: undefined }],
      ["missing plan", { ...REQUEST, plan: undefined }],
      // Would scan a disk and report nothing found, indistinguishable from clean.
      ["empty scanner list", { ...REQUEST, plan: { ...PLAN, scanners: [] } }],
      // A near-miss must never be read as the operator's attestation.
      ["liveValidated as a string", { ...REQUEST, settings: { ...SETTINGS, liveValidated: "true" } }],
      ["liveValidated false", { ...REQUEST, settings: { ...SETTINGS, liveValidated: false } }],
      // A mutable tag would make a published finding unattributable.
      ["unpinned scanner image", {
        ...REQUEST,
        settings: { ...SETTINGS, scannerImage: "sutra/agentless-scanner:0.1.0" },
      }],
      ["malformed KMS ARN", { ...REQUEST, settings: { ...SETTINGS, kmsKeyArn: "arn:aws:kms:oops" } }],
      ["malformed subnet", {
        ...REQUEST,
        settings: { ...SETTINGS, instance: { ...SETTINGS.instance, subnetId: "subnet-zzz" } },
      }],
      ["bad region", { ...REQUEST, region: "ap_south_1" }],
    ];
    for (const [label, payload] of cases) {
      const response = await signedRequest(
        baseUrl, secret, "POST", `/v1/agentless/scans/${RUN_ID}/execute`, payload,
      );
      assert.equal(response.status, 400, `${label} must be refused with 400`);
    }

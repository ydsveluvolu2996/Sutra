...
export function buildFixtureSnapshot(input: FixtureSnapshotInput): PilotSnapshot {
  ...
  // A third EC2 instance so the patch-posture fixture shows all three honest
  // states: webInstance (compliant, SSM-managed), paymentsInstance
  // (non-compliant, missing critical patches), and workerInstance (no SSM data,
  // reported as unmanaged / not-assessed — never implied compliant).
  const paymentsInstance = fixtureResource({
    accountId,
    partition,
    collectedAt,
    region: primaryRegion,
    service: "ec2",
    resourceType: "aws.ec2.instance",
    nativeId: "i-0c0ffee1d2e3f4a5b",
    arn: `arn:${partition}:ec2:${primaryRegion}:${accountId}:instance/i-0c0ffee1d2e3f4a5b`,
    name: "payments-api-01",
    state: "running",
    tags: { Name: "payments-api-01", Environment: "production", PatchGroup: "linux" },
    api: "ec2:DescribeInstances",
    configuration: {
      instanceType: "m6i.large",
      architecture: "x86_64",
      vpcId: vpc.nativeId,
      subnetId: privateSubnet.nativeId,
      securityGroupIds: [appSg.nativeId],
      privateIpAddress: "10.20.11.42",
      metadataHttpTokens: "required",
    },
  });
  // Patch-compliance posture is collected read-only as its own resource type and
  // joined to the instance by id in the posture engine. webInstance is compliant;
  // paymentsInstance is missing critical/security patches; the stopped worker has
  // no SSM patch state, so no patch-state resource is emitted for it.
  const webPatchState = fixtureResource({
    accountId,
    partition,
    collectedAt,
    region: primaryRegion,
    ...
    nativeId: webInstance.nativeId,
    arn: null,
    name: "customer-portal-01 patch state",
    state: "compliant",
    tags: {},
    api: "ssm:DescribeInstancePatchStates",
    configuration: {
      instanceId: webInstance.nativeId,
      managed: true,
      ...
      patchGroup: "linux",
      baselineId: "pb-0a1b2c3d4e5f60718",
      operation: "Scan",
      operationStartTime: "2026-07-15T04:12:00.000Z",
      ...
      missingCount: 0,
      failedCount: 0,
      notApplicableCount: 21,
      criticalMissingCount: 0,
      securityMissingCount: 0,
      otherNonCompliantCount: 0,
    },
  });
  const paymentsPatchState = fixtureResource({
    accountId,
    partition,
    collectedAt,
    region: primaryRegion,
    service: "ssm",
    resourceType: "aws.ssm.patch-state",
    nativeId: paymentsInstance.nativeId,
    arn: null,
    name: "payments-api-01 patch state",
    state: "non_compliant",
    tags: {},
    api: "ssm:DescribeInstancePatchStates",
    configuration: {
      instanceId: paymentsInstance.nativeId,
      managed: true,
      patchStateAvailable: true,
      pingStatus: "Online",
      platformType: "Linux",
      ...
      missingPatches: [
        { title: "Linux kernel security update", kbId: "USN-6013-1", classification: "Security", severity: "Critical" },
        { title: "OpenSSL security update", kbId: "USN-6039-1", classification: "Security", severity: "Critical" },
        { title: "glibc security update", kbId: "USN-6011-1", classification: "Security", severity: "Important" },
        { title: "curl security update", kbId: "USN-6022-1", classification: "Security", severity: "Important" },
      ],
    },
  });

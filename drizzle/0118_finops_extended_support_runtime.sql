-- ADV-04 durable execution receipts. Full inventory, lifecycle, pricing and
-- CUR2 evidence remains in the immutable snapshot store; this ledger retains
-- only tenant scope, bounded leases, sealed result identifiers and failures.
...
CREATE TABLE `finops_extended_support_runtime_failures` (
  ...
  FOREIGN KEY (`idempotency_key`) REFERENCES `finops_extended_support_runtime_receipts`(`idempotency_key`) ON DELETE CASCADE,
  FOREIGN KEY (`org_id`) REFERENCES `organizations`(`id`) ON DELETE CASCADE,
  FOREIGN KEY (`customer_id`) REFERENCES `customers`(`id`) ON DELETE CASCADE,
  FOREIGN KEY (`connection_id`) REFERENCES `aws_connections`(`id`) ON DELETE CASCADE,
  UNIQUE (`idempotency_key`,`job_id`,`content_sha256`),
  CHECK (length(`failure_id`) = 68 AND substr(`failure_id`,1,4) = 'esf_'
    AND substr(`failure_id`,5) NOT GLOB '*[^a-f0-9]*'),
  CHECK (length(`job_id`) = 36 AND substr(`job_id`,1,4) = 'job_'
    AND substr(`job_id`,5) NOT GLOB '*[^a-f0-9]*'),
...
--> statement-breakpoint
CREATE TRIGGER `finops_extended_support_runtime_receipts_identity_guard`
...
BEGIN SELECT RAISE(ABORT,'FINOPS_EXTENDED_SUPPORT_RUNTIME_IDENTITY_IMMUTABLE'); END;
--> statement-breakpoint
CREATE TRIGGER `finops_extended_support_runtime_receipts_transition_guard`
BEFORE UPDATE ON `finops_extended_support_runtime_receipts`
WHEN NOT (
  ...
  OR (OLD.`state` = 'FAILED' AND NEW.`state` = 'IN_PROGRESS' AND NEW.`updated_at` >= OLD.`updated_at`)
)
BEGIN SELECT RAISE(ABORT,'FINOPS_EXTENDED_SUPPORT_RUNTIME_TRANSITION_REJECTED'); END;
--> statement-breakpoint
...
BEGIN SELECT RAISE(ABORT,'FINOPS_EXTENDED_SUPPORT_RUNTIME_RECEIPT_IMMUTABLE'); END;
--> statement-breakpoint
CREATE TRIGGER `finops_extended_support_runtime_failures_update_guard`
BEFORE UPDATE ON `finops_extended_support_runtime_failures`
BEGIN SELECT RAISE(ABORT,'FINOPS_EXTENDED_SUPPORT_RUNTIME_FAILURE_IMMUTABLE'); END;
--> statement-breakpoint
CREATE TRIGGER `finops_extended_support_runtime_failures_delete_guard`
BEFORE DELETE ON `finops_extended_support_runtime_failures`
BEGIN SELECT RAISE(ABORT,'FINOPS_EXTENDED_SUPPORT_RUNTIME_FAILURE_IMMUTABLE'); END;

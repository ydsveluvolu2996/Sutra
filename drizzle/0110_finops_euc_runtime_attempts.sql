CREATE TABLE `finops_euc_runtime_attempts` (
  ...
  `boundary_sha256` text NOT NULL, `account_count` integer NOT NULL, `region_count` integer NOT NULL,
  ...
  `generation_id` text, `capture_id` text, `cur2_generation_id` text, `cur2_projection_sha256` text,
  `request_body_sha256` text, `response_body_sha256` text, `broker_key_id` text,
  ...
  FOREIGN KEY (`generation_id`) REFERENCES `finops_euc_snapshots`(`generation_id`),
  UNIQUE (`org_id`,`customer_id`,`connection_id`,`request_id`,`job_attempt`),
  CHECK (length(`execution_id`)=68 AND substr(`execution_id`,1,4)='eue_' AND substr(`execution_id`,5) NOT GLOB '*[^a-f0-9]*'),
  CHECK (length(`request_id`)=68 AND substr(`request_id`,1,4)='eur_' AND substr(`request_id`,5) NOT GLOB '*[^a-f0-9]*'),
  CHECK (length(`job_id`)=36 AND substr(`job_id`,1,4)='job_' AND substr(`job_id`,5) NOT GLOB '*[^a-f0-9]*'),
  CHECK (length(`boundary_sha256`)=64 AND `boundary_sha256` NOT GLOB '*[^a-f0-9]*'
    AND length(`content_sha256`)=64 AND `content_sha256` NOT GLOB '*[^a-f0-9]*'),
  CHECK (`generation_id` IS NULL OR (length(`generation_id`)=69 AND substr(`generation_id`,1,5)='eucg_' AND substr(`generation_id`,6) NOT GLOB '*[^a-f0-9]*')),
  CHECK (`capture_id` IS NULL OR (length(`capture_id`)=68 AND substr(`capture_id`,1,4)='euc_' AND substr(`capture_id`,5) NOT GLOB '*[^a-f0-9]*')),
  CHECK (`cur2_generation_id` IS NULL OR (length(`cur2_generation_id`)=68 AND substr(`cur2_generation_id`,1,4)='fbg_' AND substr(`cur2_generation_id`,5) NOT GLOB '*[^a-f0-9]*')),
  CHECK (`cur2_projection_sha256` IS NULL OR (length(`cur2_projection_sha256`)=64 AND `cur2_projection_sha256` NOT GLOB '*[^a-f0-9]*')),
  CHECK (`request_body_sha256` IS NULL OR (length(`request_body_sha256`)=64 AND `request_body_sha256` NOT GLOB '*[^a-f0-9]*')),
  CHECK (`response_body_sha256` IS NULL OR (length(`response_body_sha256`)=64 AND `response_body_sha256` NOT GLOB '*[^a-f0-9]*')),
  CHECK (`account_count` BETWEEN 1 AND 200 AND `region_count` BETWEEN 1 AND 50),
...
) BEGIN SELECT RAISE(ABORT,'FINOPS_EUC_RUNTIME_ATTEMPT_SCOPE_REJECTED'); END;
--> statement-breakpoint
CREATE TRIGGER `finops_euc_runtime_attempts_update_guard` BEFORE UPDATE ON `finops_euc_runtime_attempts`
BEGIN SELECT RAISE(ABORT,'FINOPS_EUC_RUNTIME_ATTEMPT_IMMUTABLE'); END;
--> statement-breakpoint
CREATE TRIGGER `finops_euc_runtime_attempts_delete_guard` BEFORE DELETE ON `finops_euc_runtime_attempts`
BEGIN SELECT RAISE(ABORT,'FINOPS_EUC_RUNTIME_ATTEMPT_IMMUTABLE'); END;

...
test("standard-2026-08.3 is immutable and leaves every mutable default untouched", () => {
  assert.match(successor, /Version: standard-2026-08\.3/u);
  assert.match(successor, /Value: standard-2026-08\.3/u);
  assert.match(successor, /PermissionPackVersion:[\s\S]*Value: standard-2026-08\.3/u);
...
test("the 08.3 deny ceiling adds exactly three reviewed reads to immutable 08.2", () => {
  ...
  assert.deepEqual(additions, [
    ...ADVANCED_READS.SutraFinopsComputeOptimizerExportReadV1,
  ]);
...
test("trust, parameters, metadata grants, and attestation are preserved from 08.2", () => {
  assert.equal(
...
test("advanced sources stay read-only and the base role still does not grant export access", () => {
  ...
  assert.doesNotMatch(
    successor,
    /(?:organizations|support|ce|compute-optimizer):(Create|Update|Delete|Enable|Disable|Export)|Action:\s*['"]?\*['"]?/u,
  );
  for (const action of [
    "s3:ListBucket",
...
test("the runbook pins source partition and Region boundaries", () => {
  assert.match(runbook, /commercial `aws` partition/u);
  assert.match(runbook, /Compute Optimizer/u);
  assert.match(runbook, /same\s+partition/u);
  assert.match(runbook, /`us-east-1`/u);
  assert.match(runbook, /fail closed/u);
  assert.match(runbook, /(?:source|export-object)\s+(?:contract|binding)/u);
});

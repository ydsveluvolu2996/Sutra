...
const revision = await readFile(
  new URL(
    "../infrastructure/finops-foundational-focus12-export-v1.1.yaml",
    import.meta.url,
  ),
  "utf8",
);
...
// contract, which the runtime pins as foundational-focus12-export-v1.
// This add-on grants six reads; ListExports is deliberately absent because the
// export ARN is a stack output, not a discovery result.
...
test("the revision accepts exactly the two enumerated reviewed ceilings", () => {
  ...
  assert.match(
    block(section(revision, "Rules"), "RequireReviewedReadCeiling"),
...
test("the revision changes nothing but the acceptance gate", () => {
  ...
  assert.equal(
    ...
    stripBlock(section(v1, "Rules"), "RequireReviewedReadCeiling"),
  );
  assert.equal(
    stripBlock(
...
test("the contract identity the runtime pins is unchanged", () => {
  assert.match(revision, /Contract: foundational-focus12-export-v1\./u);
  assert.match(revision, /AwsTable: FOCUS_1_2_AWS/u);
  assert.match(
    section(revision, "Outputs"),
    /TemplateContractVersion:[\s\S]*?Value: foundational-focus12-export-v1\n/u,
  );
  assert.match(
    section(revision, "Outputs"),
    /TemplateRevision:[\s\S]*?Value: foundational-focus12-export-v1\.1\n/u,
  );
  assert.match(
    section(revision, "Outputs"),
    /RequiredBasePermissionPackVersion:[\s\S]*?Value:\n\s+Ref: BaseCollectorPermissionPackVersion/u,
  );
  assert.match(revision, /PolicyName: SutraFoundationalFocus12ReadV1/u);
  assert.match(
    revision,
    /ExportName:[\s\S]*?Default: sutra_foundational_focus12_v1\n/u,
  );
});

test("both enumerated ceilings permit this add-on's reads without granting them", () => {
...
test("the runbook records the successor revision and keeps activation gated", () => {
  assert.match(runbook, /finops-foundational-focus12-export-v1\.1\.yaml/u);

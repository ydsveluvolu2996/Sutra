...
function group(overrides={}){return {usageAccountId:"111122223333",region:"us-east-1",clusterId:"eks-prod",namespace:"payments",controllerKind:"Deployment",controller:"payments-api",workload:"payments-api",pod:"payments-api-1",container:"api",node:"ip-10-0-1-10",nodeInstanceType:"m7g.large",nodeAvailabilityZone:"us-east-1a",nodeCapacityType:"ON_DEMAND",nodeArchitecture:"arm64",nodeOs:"linux",nodeGroup:"payments-arm",nodeGroupImage:"ami-123",allocationKind:"WORKLOAD",currency:"USD",rowCount:1,totalCost:exact("10"),componentCosts:[{component:"CPU",exact:exact("6")},{component:"RAM",exact:exact("4")}],hourlyCosts:[{windowStartIso:"2026-07-31T23:00:00.000Z",windowEndIso:"2026-08-01T00:00:00.000Z",currency:"USD",totalCost:exact("10"),componentCosts:[{component:"CPU",exact:exact("6")},{component:"RAM",exact:exact("4")}],rowCount:1}],efficiencies:[efficiency("CPU","1","2"),efficiency("RAM","50","100")],sourceRowIds:["row-1"],sourceRowsTruncated:false,...overrides};}
function snapshot(){return {schemaVersion:"sutra.kubecost-allocation.snapshot.v1",scope:{orgId:"org_a",customerId:"customer_a",connectionId:connection,partition:"aws",billingPeriod:"2026-08",activeCur2GenerationId:`fbg_${"b".repeat(64)}`,awsAccountIds:["111122223333"],clusterIds:["eks-prod","eks-dev"]},captureId:`kubecost_${"c".repeat(64)}`,state:"READY",complete:true,generatedAtIso:"2026-08-01T01:00:00.000Z",dataThroughAtIso:"2026-08-01T00:00:00.000Z",ageHours:1,exportLineage:{provider:"KUBECOST",exporterName:"exporter",exporterVersion:"1",schemaName:"sutra.kubecost-opencost-allocation",schemaVersion:"2.0.0",schemaSha256:"1".repeat(64),manifestSha256:"2".repeat(64),querySha256:"3".repeat(64),costModelSha256:"4".repeat(64),objectCount:1,versionPinnedObjectCount:1},coverage:{expectedObjects:1,processedObjects:1,failedObjects:0,expectedClusters:2,capturedClusters:2,rowsExhausted:true},rowCount:2,groupCount:2,categoryTotals:[{category:"WORKLOAD_ALLOCATION",currency:"USD",exact:exact("14"),rowCount:2}],groups:[group(),group({usageAccountId:"444455556666",clusterId:"eks-dev",namespace:"analytics",workload:"worker",pod:"worker-1",container:"worker",nodeCapacityType:"SPOT",nodeInstanceType:"c7g.large",totalCost:exact("4"),componentCosts:[{component:"CPU",exact:exact("3")},{component:"RAM",exact:exact("1")}],hourlyCosts:[{windowStartIso:"2026-07-31T23:00:00.000Z",windowEndIso:"2026-08-01T00:00:00.000Z",currency:"USD",totalCost:exact("4"),componentCosts:[{component:"CPU",exact:exact("3")},{component:"RAM",exact:exact("1")}],rowCount:1}],sourceRowIds:["row-2"]})],reconciliation:{state:"MATCHED",authoritativeSpendSource:"AWS_CUR2_ACTIVE_GENERATION",presentationPolicy:"ATTRIBUTION_VIEW_ONLY_DO_NOT_ADD_TO_CUR2",toleranceMicros:"0",currencies:[{currency:"USD",kubecostTotal:exact("14"),cur2TotalMicros:"14000000",delta:exact("0"),withinTolerance:true}]},limitations:["Allocation only."]};}
...
test("Kubecost dashboard uses exact aggregation, filters, pivots and usage-vs-request efficiency",async()=>{
  ...
  assert.deepEqual(report.executiveSummary.componentCosts.find((item)=>item.component==="CPU")?.totalCost,exact("9"));
  assert.deepEqual(report.hourlyCosts[0]?.totalCost,exact("14"));
  ...
  assert.deepEqual(report.eksBreakdown.capacityTypes.map((item)=>item.identity),["ON_DEMAND","SPOT"]);
  assert.deepEqual(filtered.eksBreakdown.instanceTypes.map((item)=>item.identity),["c7g.large"]);
});
...
test("repository and API enforce normalized immutable persistence and same-tenant reads",async()=>{
  ...
  assert.match(route,/requireApiSession\(request\)/u);assert.match(route,/getConnectionForOrg\(authenticated\.subject\.orgId/u);assert.match(route,/assertSessionCapability\(authenticated, "connection:read", connection\.customerId\)/u);assert.match(route,/providerAdapterAvailable: true/u);assert.match(route,/runtimeState/u);assert.match(route,/do not add it to authoritative CUR2 spend/u);
  assert.equal((route.match(/officialDefinition: KUBECOST_OFFICIAL_DEFINITION/gu)??[]).length,2);
});

test("native Kubecost UI renders official sections, reconciliation and honest unavailable states",async()=>{
  const vite=await createServer({root,configFile:false,logLevel:"silent",plugins:[react()],server:{middlewareMode:true}});try{const dashboardModule=await vite.ssrLoadModule("/app/costs/finops-kubecost-allocation-dashboard.tsx");const definitionModule=await vite.ssrLoadModule("/lib/finops-kubecost-official-definition.ts");const projection=(await import("../lib/finops-kubecost-dashboard.ts")).buildKubecostDashboard(snapshot());
    const report={...projection,connectionId:connection,sourceState:"partial",officialDefinition:definitionModule.KUBECOST_OFFICIAL_DEFINITION,history:[],freshness:{ageHours:1},evidence:{activeCur2GenerationId:snapshot().scope.activeCur2GenerationId},collection:{jobContractAvailable:true,providerAdapterAvailable:true,runtimeState:"collecting",sharedWorkerRegistered:false,reason:"AWAITING_SHARED_REGISTRY_HOOK"},disclosures:["Allocation only."]};
    const html=renderToStaticMarkup(createElement(dashboardModule.FinopsKubecostAllocationReportView,{report,filters:{},onFiltersChange:()=>undefined}));for(const text of ["Allocation, not additional spend","Collection runtime:","collecting","Official Kubecost dashboard tabs","Executive Summary","CPU cost","Total cost by account","Daily allocated-cost trend","Workloads Explorer","EKS Breakdown","Capacity type","Instance type","Showback / chargeback evidence","usage vs requests","Reconciliation, provenance"] )assert.match(html,new RegExp(text,"iu"));assert.doesNotMatch(html,/sample|fixture|placeholder/iu);
    const officialHtml=renderToStaticMarkup(createElement(dashboardModule.KubecostOfficialSourcePanel,{definition:definitionModule.KUBECOST_OFFICIAL_DEFINITION}));for(const text of ["Official Kubecost source audit","three purpose-level tabs","62-column SPICE dataset","QuickSight template is service-hosted","Executive Summary","Workloads Explorer","EKS Breakdown","Supplemental OpenCost boundary","exact object totals: null"] )assert.match(officialHtml,new RegExp(text,"iu"));assert.match(officialHtml,/SUPPLEMENTAL|not supported/iu);
    const nullHtml=renderToStaticMarkup(createElement(dashboardModule.FinopsKubecostAllocationDashboard,{connectionId:null}));assert.match(nullHtml,/Official Kubecost source audit/u);assert.match(nullHtml,/Unavailable.*Connect an active AWS trust-role account/u);

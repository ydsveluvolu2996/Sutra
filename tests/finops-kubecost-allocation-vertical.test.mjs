...
test("native Kubecost UI renders official sections, reconciliation and honest unavailable states",async()=>{
  const vite=await createServer({root,configFile:false,logLevel:"silent",plugins:[react()],server:{middlewareMode:true}});try{const dashboardModule=await vite.ssrLoadModule("/app/costs/finops-kubecost-allocation-dashboard.tsx");const definitionModule=await vite.ssrLoadModule("/lib/finops-kubecost-official-definition.ts");const projection=(await import("../lib/finops-kubecost-dashboard.ts")).buildKubecostDashboard(snapshot());
    const report={...projection,connectionId:connection,sourceState:"partial",officialDefinition:definitionModule.KUBECOST_OFFICIAL_DEFINITION,history:[],freshness:{ageHours:1},evidence:{activeCur2GenerationId:snapshot().scope.activeCur2GenerationId},collection:{jobContractAvailable:true,providerAdapterAvailable:false,reason:"KUBECOST_SIGNED_VERSIONED_EXPORT_RUNTIME_NOT_REGISTERED"},disclosures:["Allocation only."]};
    const html=renderToStaticMarkup(createElement(dashboardModule.FinopsKubecostAllocationReportView,{report,filters:{},onFiltersChange:()=>undefined}));for(const text of ["Allocation, not additional spend","Official Kubecost dashboard tabs","Executive Summary","CPU cost","Total cost by account","Hourly allocated-cost trend","Workloads Explorer","EKS Breakdown","Showback / chargeback evidence","usage vs requests","instance-type breakdown unavailable","Reconciliation, provenance"] )assert.match(html,new RegExp(text,"iu"));assert.doesNotMatch(html,/sample|fixture|placeholder/iu);
    ...
    const nullHtml=renderToStaticMarkup(createElement(dashboardModule.FinopsKubecostAllocationDashboard,{connectionId:null}));assert.match(nullHtml,/Official Kubecost source audit/u);assert.match(nullHtml,/Connect an active AWS trust-role account/u);

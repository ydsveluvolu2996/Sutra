...
test("exports exact filtered transfer evidence and neutralizes formulas", () => {
  ...
  assert.match(csv, /"source_location"/u);
  assert.match(csv, /"North America"/u);
  assert.match(csv, /"AWS Outbound"/u);
  assert.match(csv, /"provider_product_code"/u);
  assert.match(csv, /"provider_operation"/u);

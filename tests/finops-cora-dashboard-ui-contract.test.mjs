...
test("CORA report actually renders filters, honest disclosures, history, drilldown and evidence", async () => {
  ...
  const report = {
    ...
    officialSheetCoverage: [
      { sheet: "Summary", status: "IMPLEMENTED", localEvidence: "Resource-deduplicated summaries", limitation: null },
      { sheet: "Usage Optimization", status: "IMPLEMENTED", localEvidence: "Usage actions", limitation: null },
  ...
  for (const text of ["not realized savings", "rate estimates are not adjusted", "Cost allocation tag", "Resource ID", "FinopsException", "Drill down", "Daily evidence history", "About · Evidence and coverage", "Export visible rows", "Coverage is partial", "Usage Optimization", "Savings Plans", "Reserved Instances", "Resource-deduplicated opportunity summaries"]) assert.match(html, new RegExp(text, "iu"));
  } finally {
    await vite.close();
  }
});

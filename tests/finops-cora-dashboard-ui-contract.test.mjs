...
test("CORA report actually renders filters, honest disclosures, history, drilldown and evidence", async () => {
  ...
  const filters = { accountId: null, optimizationClass: null, actionType: null, region: null, implementationEffort: null, workflowStatus: null, currencyCode: null, tagKey: null, tagValue: null };
  ...
  for (const text of ["not realized savings", "rate estimates are not adjusted", "Cost allocation tag", "Drill down", "Daily evidence history", "Evidence and coverage", "Export visible rows", "Coverage is partial"]) assert.match(html, new RegExp(text, "iu"));
  } finally {
    await vite.close();
  }
});

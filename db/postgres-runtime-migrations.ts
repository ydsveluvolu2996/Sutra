...
const MIGRATION_ID = "0000_sutra_baseline";

let schemaReady: Promise<void> | undefined;

async function baselineSha256(): Promise<string> {
  const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(postgresBaselineSql));
...
export function ensurePostgresRuntimeSchema(db: D1Database): Promise<void> {
  ...
  const attempt = (async () => {
    ...
    ).bind(MIGRATION_ID).first<{ migration_id: string; migration_sha256: string | null }>();
    if (applied === null) throw new Error("PostgreSQL is not migrated; run pnpm db:postgres:migrate with the owner role");
    if (applied.migration_sha256 !== await baselineSha256()) {
      throw new Error(`Applied PostgreSQL migration ${MIGRATION_ID} failed its immutable checksum`);
    }
  })();
  schemaReady = attempt;
  void attempt.catch(() => {

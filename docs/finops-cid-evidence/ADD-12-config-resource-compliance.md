...
production-accepted capability. The permanent collector adapter is not
implemented, so API activation is
deliberately `available: false` with
`AWS_CONFIG_COLLECTOR_ADAPTER_NOT_IMPLEMENTED`. No fixture or sample data is
substituted.

Remaining gates:

1. Register the reserved migrations in both runtime migration registries and
   schema declarations.
2. Implement and provider-test the exact bounded aggregator, Organizations,
   recorder, and rule-lifecycle adapters, including empty-page pagination.
3. Bind optional Config S3 delivery objects and reconciled active CUR 2.0 rows
   to the capture without accepting browser-supplied AWS scope.
4. Extend the minimized projection for required-tag compliance and the
...
5. Run controlled multi-account/Region AWS acceptance, cross-tenant rejection,

...
| Collector adapter | `services/aws-collector/src/scad-cur2-provider-adapter.ts` (S3 `readObject` precedent) | `REPAIR` | Reads CUR2 objects but does not enumerate exports or emit observations | Extend pattern into a new discovery adapter |

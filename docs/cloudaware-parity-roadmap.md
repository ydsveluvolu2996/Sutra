...
| **CMDB inventory** | Persistent snapshots for EC2/VPC, subnet, security group, EBS, ENI, ALB/NLB, KMS, DynamoDB, ECR, S3, RDS, IAM account posture, CloudTrail and native security-service coverage; normalized resources, provenance, bounded relationships, freshness, history and export. Deterministic durable-job retry/DLQ, governed local retention, and a PostgreSQL dump wrapper are delivered locally. Lambda remains opt-in/off because `ListFunctions` can expose environment values. | Deploy the queue workers and monitoring; prove backup restore, authenticated ingestion, quotas and atomic reconciliation under load | Broader AWS service and CI-type coverage, event-assisted change capture, ownership, custom fields, reconciliation policies and supported search/query API | Production gate → planned expansion |

...
| ADV-02 | Compute Optimizer Dashboard | `compute_optimizer` | `PARTIAL_PIPELINE` | [Current evidence](finops-cid-evidence/ADV-02-compute-optimizer.md): the pinned public source proves 9 export module families and 14 preview visual purposes; its QuickSight definition is not publicly committed, so exact sheet/control totals remain explicitly unavailable. Immutable organization S3-export history, same-tenant API, native rightsizing/savings/risk/ownership UI, a strict hash-addressed AWS CSVW parser, and the immutable `standard-2026-08.3` read-only discovery permission candidate now exist. Production Region-specific export-plan binding, exact S3/KMS object policy, resource mappers, durable handler, provider validation, exact template authorization and live acceptance remain. Discovery and direct recommendation APIs are never substituted for export history. |

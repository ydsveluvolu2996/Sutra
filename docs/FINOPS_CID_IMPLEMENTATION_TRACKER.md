...
| Advanced | 13 | 2 | 11 | 0 | 0 | 0 | 0 |
| Additional | 13 | 2 | 11 | 0 | 0 | 0 | 0 |
| **Total** | **29** | **7** | **22** | **0** | **0** | **0** | **0** |

### Current release-scope view

| Release scope | Catalog rows | Candidate | Partial pipeline | Local verified | Live accepted |
|---|---:|---:|---:|---:|---:|
| In scope — AWS-backed dashboards | 27 | 7 | 20 | 0 | 0 |
| Excluded from this build — Azure CID and GCP CID | 2 | 0 | 2 | 0 | 0 |
| **Official catalog retained** | **29** | **7** | **22** | **0** | **0** |

The seven candidates are CUDOS, Cost Intelligence, KPI and Modernization, Cost
Anomaly, AWS News Feeds, Trends, and Data Transfer. The twenty-two partial pipelines are Trusted
Advisor Organizational, Compute Optimizer, Extended Support Cost Projection, Graviton Savings, Health Events, AWS Budgets, Support Cases Radar,
...
| ADV-02 | Compute Optimizer Dashboard | `compute_optimizer` | `PARTIAL_PIPELINE` | [Current evidence](finops-cid-evidence/ADV-02-compute-optimizer.md): the pinned public source proves 9 module families and 14 preview visual purposes; the unpublished QuickSight definition keeps exact sheet/control totals explicitly unavailable. Local code now has the canonical eight-export materialization projection, replay-safe durable launch ledger, exact-ID Describe barrier, version-bound S3 object reader, strict CSVW parser/mappers, all-Region coordinator, immutable exact-generation persistence, registered exact materialization worker, same-tenant v2 API, and native exact-micros UI for all 14 purposes. The exact `.8.5` activation manifest is exposed only through a signed, tenant-bound collector route; the default broker performs identity-only STS attestation and the app transport enforces signed responses, deadlines, and a streaming 64 KiB ceiling. Partial/ambiguous runs never head, discovery/direct APIs never substitute, alternative savings channels and currencies never merge. The durable capability/outbox scheduler and crash-safe sealed-reference handoff, secret/IAM activation, controlled multi-Region provider reconciliation, two-tenant/live acceptance, and exact authorized QuickSight geometry remain; therefore maturity stays partial and no release image is claimed. |

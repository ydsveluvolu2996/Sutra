...
|---|---|---|---|---|
| FND-01 | CUDOS Dashboard | `cudos` | `LOCAL_VERTICAL_CANDIDATE` | [G0–G6 evidence](finops-cid-evidence/FND-01-cudos.md): monthly/UTC-weekly/daily trends, FOCUS category rankings, EUC and GameTech/Media modules, active CUR2 completeness disclosure and 69 focused tests are present. Provider telemetry/recommendations, commitment expiry/purchase context, exact QuickSight visual parity, exact-tree/two-tenant reconciliation and live acceptance remain. |
...
| ADV-03 | Cost Anomaly Dashboard | `cost_anomaly` | `LOCAL_VERTICAL_CANDIDATE` | [G0–G6 evidence](finops-cid-evidence/ADV-03-cost-anomaly.md): bounded null-aware provider analysis, monthly actual/expected evidence, four-dimensional root-cause contribution, anomaly/score/impact/account/service/Region/usage/assessment/monitor/window filters, redacted monitor/subscription coverage and safe export are present. Controlled payer/two-tenant reconciliation, exact-tree and live acceptance remain. |
| ADV-04 | Extended Support Cost Projection | `extended_support_projection` | `PARTIAL_PIPELINE` | [Current evidence](finops-cid-evidence/ADV-04-extended-support.md): five-service multi-account/Region collection contract, identity-only daily scheduling, sealed replay-safe durable handler boundary, immutable READY-only history, same-tenant API, exact signed-micro output, and native 3/6/12-month UI exist; credential-owning provider adapter, permanent replay/shared-handler registration, IAM, decimal-string provider money inputs, real calendar/rate/CUR2 reconciliation, and live acceptance remain. |
| ADV-05 | Graviton Savings Dashboard | `graviton_savings` | `PARTIAL_PIPELINE` | [Current evidence](finops-cid-evidence/ADV-05-graviton-savings.md): EC2/ASG, RDS/Aurora, OpenSearch and ElastiCache contracts, prevalidated identity-only daily scheduling, validated signed replay receipts, exact-micro CUR2 economics, immutable history, same-tenant API, native usage/opportunity/trend/drilldown UI and safe export exist. Shared handler/archive/sealer/credential adapter, exact seven-sheet controls, provider reconciliation and live acceptance remain. |
...
| ADV-09 | AWS Support Cases Radar Dashboard | `support_cases_radar` | `PARTIAL_PIPELINE` | [Current evidence](finops-cid-evidence/ADV-09-support-cases-radar.md): pinned official inventory, privacy-minimized immutable incremental history, exact entitlement-probe request, authenticated broker transport, strict five-attempt/prevalidated daily scheduler contract, same-tenant API, native case-age/response-cadence/topic visuals, plan states, and explicit optional-summary state exist; production registration, credential-owning AWS adapter, exact QuickSight geometry and provider acceptance remain. |
| ADV-10 | ResilienceVue Dashboard | `resiliencevue` | `PARTIAL_PIPELINE` | [Current evidence](finops-cid-evidence/ADV-10-resiliencevue.md): the pinned four-sheet inventory, replay-safe target/window runtime, immutable multi-account/Region history, canonical last-assessment controls, assessed/in-policy posture, latest-ten score trend, dimension-level RPO/RTO evidence and SOP/alarm/FIS status visuals exist; estimated cost/optimization/architecture require a versioned schema, while credential-broker registration and provider acceptance remain. |
...
| ADD-01 | CORA Dashboard | `cora` | `PARTIAL_PIPELINE` | [Current evidence](finops-cid-evidence/ADD-01-cora.md) and [export activation contract](finops-cid-evidence/ADD-01-cora-export-activation.md): execution-specific Cost Optimization Hub export materialization, immutable complete-only object history, deterministic retry identity, resource-safe Usage/Rate deduplication, exact recommendation filters, same-tenant API and five official native sheet areas exist. SP/RI comparison dimensions, credential-owning S3/Parquet adapter, durable handler, exact-tree/provider and live acceptance remain. |

...
`1eb00d50f6b360f0582bf0dbbe15d9fd2478112e`** — all three Foundational
dashboards were closed through a shared adversarial pass: FND-01 now withholds
Bedrock ratios when any compatible quantity is missing and renders structured
source formats truthfully; FND-02 adds bounded canonical OPTICS controls and an
exact currency/unit-separated MoM quantity pivot; FND-03 adds governed mutation
and complete history states for all 19 goals. Exact-SHA standing-PR CI passed in
[run 32493632168](https://github.com/ydsveluvolu2996/Sutra/actions/runs/32493632168).
All three remain `LOCAL_VERTICAL_CANDIDATE`; no main merge, deployment,
provider acceptance, or live acceptance is claimed.
...
| Advanced | 13 | 11 | 2 | 0 | 0 | 0 | 0 |
| Additional | 13 | 2 | 11 | 0 | 0 | 0 | 0 |
| **Total** | **29** | **16** | **13** | **0** | **0** | **0** | **0** |

### Current release-scope view

| Release scope | Catalog rows | Candidate | Partial pipeline | Local verified | Live accepted |
|---|---:|---:|---:|---:|---:|
| In scope — AWS-backed dashboards | 27 | 16 | 11 | 0 | 0 |
| Excluded from this build — Azure CID and GCP CID | 2 | 0 | 2 | 0 | 0 |
| **Official catalog retained** | **29** | **16** | **13** | **0** | **0** |

The sixteen candidates are CUDOS, Cost Intelligence, KPI and Modernization, Cost
Anomaly, Compute Optimizer, Extended Support Cost Projection, Graviton Savings,
Health Events, AWS News Feeds, AWS Budgets, Support Cases Radar, ResilienceVue,
End User Computing, Data Collection Monitor, Trends, and Data Transfer. The
thirteen partial pipelines are Trusted Advisor Organizational, CORA, FOCUS,
Config Resource Compliance, Pricing Change Analysis, Media Services Insights,
...
| ADV-05 | Graviton Savings Dashboard | `graviton_savings` | `LOCAL_VERTICAL_CANDIDATE` | [Current evidence](finops-cid-evidence/ADV-05-graviton-savings.md): the pinned v3.0.2 inventory maps 7 sheets, 122 visuals and 53 controls. Exact `.8.12` permissions, bounded credential-owning provider route, signed broker, durable CAS lease/replay, all three migration registries, daily shared handler, immutable complete-only history, server-scoped API, and native evidence-honest UI are wired and passed the 2026-08-21 local G0–G6 matrix. Provider reconciliation, richer standalone mapping/service controls, signed-in two-tenant exact-tree acceptance, release gates, and live acceptance remain `PENDING_EXTERNAL_ACCEPTANCE`. |

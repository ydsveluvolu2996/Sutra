...
| ADV-02 | Compute Optimizer Dashboard | `compute_optimizer` | `PARTIAL_PIPELINE` | [Current evidence](finops-cid-evidence/ADV-02-compute-optimizer.md): the pinned public source proves 9 module families and 14 preview visual purposes; the unpublished QuickSight definition keeps exact sheet/control totals explicitly unavailable. Local code now has the canonical eight-export materialization projection, replay-safe durable launch ledger, exact-ID Describe barrier, version-bound S3 object reader, strict CSVW parser/mappers, all-Region coordinator, immutable exact-generation persistence, same-tenant v2 API, and native exact-micros UI for all 14 purposes. Partial/ambiguous runs never head, discovery/direct APIs never substitute, alternative savings channels and currencies never merge. Production scheduler/runtime registration, secret/IAM activation, controlled multi-Region provider reconciliation, two-tenant/live acceptance, and exact authorized QuickSight geometry remain; therefore maturity stays partial and no release image is claimed. |

...
export function ResourceBrowser() {
  ...
  return <>
    ...
    <div className="trust-strip" role="note"><span className="trust-icon">✓</span><span><strong>{snapshotOriginLabel(state.activeSnapshot.origin)}.</strong> This record belongs to immutable snapshot <code>{compactIdentifier(state.activeSnapshot.id, 14)}</code> collected {formatTimestamp(state.activeSnapshot.collectedAt)}.</span></div>

    <section className="resource-identity-grid">
      <article className="panel resource-identity-card"><span className="service-chip">{resource.service.toUpperCase()}</span><h2>{resource.resourceType}</h2><p>{resource.nativeId}</p><dl><div><dt>Account</dt><dd>{connection.awsAccountId}</dd></div><div><dt>Region</dt><dd>{resource.region}</dd></div><div><dt>State</dt><dd>{resource.state || "observed"}</dd></div><div><dt>ARN</dt><dd title={resource.arn ?? "Not supplied"}>{resource.arn ? compactIdentifier(resource.arn, 34) : "Not supplied"}</dd></div></dl></article>
      ...
      <article className="panel resource-provenance-card"><p className="eyebrow">Evidence provenance</p><h2>{resource.source.api}</h2><dl><div><dt>Observed</dt><dd>{formatTimestamp(resource.source.collectedAt)}</dd></div><div><dt>Content hash</dt><dd title={resource.contentSha256}>{compactIdentifier(resource.contentSha256, 18)}</dd></div><div><dt>Snapshot hash</dt><dd title={state.activeSnapshot.snapshotSha256}>{compactIdentifier(state.activeSnapshot.snapshotSha256, 18)}</dd></div></dl></article>
    </section>

...
function KubernetesTabs({ active }: { readonly active: KubernetesView }) {
  return (
    <nav className="kubernetes-tabs" aria-label="Kubernetes workspace sections">
      <a aria-current={active === "overview" ? "page" : undefined} href="/kubernetes">Overview</a>
      <a aria-current={active === "inventory" ? "page" : undefined} href="/kubernetes/inventory">Inventory</a>
      <a aria-current={active === "security" ? "page" : undefined} href="/kubernetes/security">Security</a>
    </nav>
  );
}
  ...
  return (
    <section className="panel empty-workspace kubernetes-empty">
      ...
      <div className="heading-actions">
        <a className="button button-primary" href="/onboard">{connected ? "Review AWS connection" : "Onboard AWS account"}</a>
        <a className="button button-secondary" href="/roadmap">Review collector roadmap</a>
      </div>
    </section>
  );
}
    ...
    <details className="kubernetes-resource-card">
      ...
      <div className="kubernetes-resource-detail">
        ...
        <div className="kubernetes-detail-actions">
          <a className="text-link" href={`/cmdb/resource?key=${encodeURIComponent(record.resource.resourceKey)}`}>Open Resource 360 →</a>
          {record.findings.length > 0 ? <a className="text-link" href="/kubernetes/security">Review security findings →</a> : null}
        </div>
      </div>
    </details>
  );
  ...
  return (
    <>
      ...
      <section className="panel kubernetes-cluster-panel">
        <div className="panel-heading"><div><p className="eyebrow">Cluster drilldown</p><h2>Observed cluster resources</h2></div><a className="text-link" href="/kubernetes/inventory">Open full inventory →</a></div>
        {observedClusterRecords.length > 0 ? <div className="kubernetes-cluster-grid">{observedClusterRecords.map((cluster) => {
          ...
          return <article key={cluster.resource.resourceKey}>
            ...
            <details><summary>Show related resources</summary><ul>{related.slice(0, 8).map((record) => <li key={record.resource.resourceKey}><a href={`/cmdb/resource?key=${encodeURIComponent(record.resource.resourceKey)}`}>{record.kind} · {record.displayName}</a></li>)}</ul></details>
          </article>;
  ...
  return (
    <details className="finding-item">
      ...
      <div className="finding-detail">
        ...
        <div><p className="eyebrow">Suggested remediation</p><p>{finding.remediation}</p>{record ? <a className="text-link" href={`/cmdb/resource?key=${encodeURIComponent(record.resource.resourceKey)}`}>Open affected resource →</a> : null}</div>
      </div>
    </details>
  );
}
  ...
  return (
    <>
      <section className="page-heading kubernetes-heading">
        ...
        <div className="heading-actions"><a className="button button-secondary" href="/onboard">AWS trust onboarding</a><a className="button button-primary" href="/cmdb">Open source CMDB</a></div>
      </section>
      ...
      {error ? <div className="page-alert page-alert-error" role="alert"><strong>Kubernetes evidence is unavailable</strong><span>{error}</span><button onClick={() => void refresh()} type="button">Retry</button></div> : null}
      {loading ? <div className="loading-state" role="status"><span className="loading-spinner" />Loading authorized Kubernetes evidence…</div> : null}
      {!loading && projection.records.length === 0 ? <EmptyKubernetesEvidence connected={connection !== null} /> : null}
      {!loading && projection.records.length > 0 ? (
        view === "overview" ? <Overview projection={projection} /> :
          view === "inventory" ? <Inventory projection={projection} /> :
            <Security projection={projection} />
      ) : null}
      {!loading ? <section className="panel kubernetes-coverage">

...
function flagClass(flag: CiemFlag): string {
  ...
  if (flag === "secrets-access" || flag === "pod-exec" || flag === "wildcard-verb" || flag === "wildcard-resource") return "compliance-status-unknown";
    ...
    <>
      ...
      {!loading && !kubernetes.loading ? <>
        <section className="inventory-stats">
          ...
          <article><small>Reach AWS</small><strong>{report.totals.awsReachable}</strong><span>{report.totals.awsWrite} with write access</span></article>
        </section>
          ...
          {subjects.length > 0 ? <div className="ciem-list">
            {subjects.map((subject) => <article className="ciem-card" key={subject.subject}>
              <div className="ciem-head">
                <div><strong>{subject.subject}</strong><small>{subject.boundRoles.length} bound role{subject.boundRoles.length === 1 ? "" : "s"}: {subject.boundRoles.join(", ") || "none resolved"}</small></div>
              ...
              {subject.awsReach !== null ? <div className="ciem-aws">
                <p className="eyebrow">AWS reach via {subject.awsReach.roleArn}</p>

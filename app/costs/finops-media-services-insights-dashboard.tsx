...
export function MediaServicesInsightsReportView({ report,filters,onFiltersChange }: { readonly report: MediaServicesEnvelope; readonly filters: MediaServicesDashboardFilters; readonly onFiltersChange: (filters: MediaServicesDashboardFilters) => void }) {
  ...
  return <section className={styles.root} aria-label="AWS Media Services Insights dashboard">
    ...
    <div className={styles.cards} aria-label="Official Media Services Insights coverage">{["Executive Summary","MediaLive Reservation & Savings","MediaConvert","MediaConnect","MediaLive","MediaTailor","MediaPackage","Raw Data","About"].map((sheet) => <article key={sheet}><strong>{sheet}</strong><small>{sheet === "MediaLive Reservation & Savings" ? "PARTIAL · governed comparison pricing required" : "EVIDENCE VIEW"}</small></article>)}</div>
...
export function FinopsMediaServicesInsightsDashboard({ connectionId }: { readonly connectionId: string | null }) {
  ...
  const [state,setState] = useState<{ report: MediaServicesEnvelope | null; error: string | null; configurationRequired: boolean }>({ report: null,error: null,configurationRequired: false });
  useEffect(() => {
    ...
    fetch(`/api/v1/finops/media-services-insights?${parameters.toString()}`,{ signal: controller.signal,headers: { Accept: "application/json" } })
      .then(async (response) => { if (!response.ok) throw new Error("Media Services Insights request failed"); return response.json() as Promise<MediaServicesEnvelope | { readonly dashboard: null }>; })
      .then((report) => { if ("dashboard" in report && report.dashboard === null) setState({ report: null,error: null,configurationRequired: true }); else setState({ report: report as MediaServicesEnvelope,error: null,configurationRequired: false }); })
      .catch((error: unknown) => { if (!controller.signal.aborted) setState({ report: null,error: error instanceof Error ? error.message : "Media Services Insights request failed",configurationRequired: false }); });
    return () => controller.abort();
  },[connectionId,filters]);
  if (connectionId === null) return <div role="status" className={`${styles.state} ${styles.warning}`}>Connect an active AWS trust-role account before collecting Media Services evidence.</div>;
  if (state.configurationRequired) return <div role="status" className={`${styles.state} ${styles.warning}`}>The governed Media Services collector has not persisted a complete accepted head for this connection.</div>;
  if (state.error !== null) return <div role="alert" className={`${styles.state} ${styles.error}`}>{state.error}</div>;
  if (state.report === null || state.report.connectionId !== connectionId) return <div role="status" className={styles.state}>Loading Media Services and CUR2 evidence…</div>;
  return <MediaServicesInsightsReportView report={state.report} filters={filters} onFiltersChange={setFilters} />;
}

...
import { buildCommitmentCoverage } from "../../../../../lib/finops-commitments";
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    if (!CONNECTION_ID.test(connectionId) || (period !== null && !BILLING_PERIOD.test(period))) {
      throw Object.assign(new Error("The FinOps commitments request is invalid"), { code: "INVALID_INPUT" });
    }
    ...
    // A real clock, computed in the route so the engine stays pure and testable.
    ...
    if (selected === null) {
      return jsonResponse({
        ...
        periods,
        period: null,
        commitments: buildCommitmentCoverage([], nowIso),
      });
    }
    ...
    return jsonResponse({
      ...
      commitments: buildCommitmentCoverage(lines, nowIso),
    });
  } catch (error) {

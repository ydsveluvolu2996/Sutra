...
import { AZURE_CID_OFFICIAL_DEFINITION } from "../../../../../lib/finops-azure-cid-official-definition";
import { AZURE_CID_RUNTIME_BINDING } from "../../../../../lib/finops-azure-cid-runtime-binding";
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    if (query.sourceId === null) return jsonResponse({ schema: "sutra.finops-azure-cid-dashboard.v1", sourceId: null, sourceState: "configuration_required", officialDefinition: AZURE_CID_OFFICIAL_DEFINITION, dashboard: null, availableSources, activation: { ready: false, reason: availableSources.length === 0 ? "AZURE_SOURCE_NOT_REGISTERED" : "AZURE_SOURCE_NOT_SELECTED" } });
    const source = authorizedSources.find((candidate) => candidate.scope.sourceId === query.sourceId);
    if (source === undefined) return jsonResponse({ schema: "sutra.finops-azure-cid-dashboard.v1", sourceId: query.sourceId, sourceState: "configuration_required", officialDefinition: AZURE_CID_OFFICIAL_DEFINITION, dashboard: null, availableSources, activation: { ready: false, reason: "AZURE_SOURCE_NOT_REGISTERED" } });
    if (source.status !== "active" || source.activationReason !== "READY") return jsonResponse({ schema: "sutra.finops-azure-cid-dashboard.v1", sourceId: query.sourceId, sourceState: "configuration_required", officialDefinition: AZURE_CID_OFFICIAL_DEFINITION, dashboard: null, availableSources, activation: { ready: false, reason: source.activationReason } });
    ...
    if (selected === null) return jsonResponse({ schema: "sutra.finops-azure-cid-dashboard.v1", sourceId: query.sourceId, sourceState: "configuration_required", officialDefinition: AZURE_CID_OFFICIAL_DEFINITION, dashboard: null, availableSources, activation: { ready: false, reason: "AZURE_EXPORT_DELIVERY_NOT_OBSERVED" } });

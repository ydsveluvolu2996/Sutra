...
function invalidRequest(): never {
  throw Object.assign(
    new Error("The AWS Config compliance request is invalid"),
...
function parseQuery(request: Request) {
  ...
  for (const key of ALLOWED)
    if (values.getAll(key).length > 1) invalidRequest();
  ...
  if (
    ...
    (complianceType !== null &&
      !["COMPLIANT", "NON_COMPLIANT", "NO_RESULTS"].includes(complianceType)) ||
  ...
  try {
    ...
    if (active === null) {
      ...
      return jsonResponse({
        ...
        sourceState,
        officialDefinition: AWS_CONFIG_COMPLIANCE_OFFICIAL_DEFINITION,
        dashboard: null,
        latestAttempt:
          latest === null
            ? null
    ...
    const newerIncomplete =
      latest !== null &&
      latest.snapshotId !== active.snapshotId &&
    ...
    const rules = snapshot.rules.filter(
      (rule) =>
        ...
        (query.complianceType === null ||
          rule.complianceType === query.complianceType) &&
        (query.resourceType === null ||
          rule.resourceTypes.includes(query.resourceType)),
    );
    const evaluations = snapshot.evaluations.filter(
      (evaluation) =>
        (query.accountId === null ||
          evaluation.accountId === query.accountId) &&
        (query.region === null || evaluation.region === query.region) &&
        (query.ruleName === null || evaluation.ruleName === query.ruleName) &&
        (query.complianceType === null || query.complianceType === "NO_RESULTS"
          ? query.complianceType === null
          : evaluation.complianceType === query.complianceType) &&
        (query.resourceType === null ||
          evaluation.resourceType === query.resourceType),
    );
    const inventory = snapshot.resourceInventory.filter(
      (resource) =>
        (query.accountId === null || resource.accountId === query.accountId) &&
        (query.region === null || resource.region === query.region) &&
        (query.resourceType === null ||
          resource.resourceType === query.resourceType),
    );
    ...
    const sourceState = newerIncomplete
      ? latest?.state === "FAILED"
        ? "failed"
        : "partial"
      : currentAgeHours === null || currentAgeHours > FRESHNESS_SLA_HOURS
        ? "stale"
        : aFilterIsActive &&
            rules.length === 0 &&
    ...
    return jsonResponse({
      ...
      conformancePacksTruncated:
        snapshot.conformancePacks.length > RESULT_BOUND,
      ...
      history: history.slice(0, 12).map((item) => ({
        ...
        rules: item.snapshot.counts.rules,
        nonCompliantResources: item.snapshot.counts.nonCompliantResources,
      })),
      latestAttempt:
        latest === null
          ? null

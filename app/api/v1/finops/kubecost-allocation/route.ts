...
import { KUBECOST_PRODUCTION_COMPOSITION_STATUS } from "../../../../../lib/finops-kubecost-production-composition";
...
const ALLOWED = new Set(["connectionId","accountId","clusterId","namespace","controllerKind","controller","workload","nodeCapacityType","nodeInstanceType","allocationKind","currency","limit","cursor"]);
...
function parse(request: Request): { readonly connectionId: string; readonly filters: KubecostDashboardFilters } {
  ...
  const workload = values.get("workload") ?? undefined; const nodeCapacityType = values.get("nodeCapacityType") ?? undefined;
  const nodeInstanceType = values.get("nodeInstanceType") ?? undefined; const allocationKind = values.get("allocationKind") ?? undefined;
  ...
  if (!CONNECTION_ID.test(connectionId) || (accountId !== undefined && !ACCOUNT.test(accountId))
    || [clusterId, namespace, controllerKind, controller, workload, nodeCapacityType, nodeInstanceType].some((item) => item !== undefined && !SAFE.test(item))
  ...
  return { connectionId, filters: { accountId, clusterId, namespace, controllerKind, controller, workload, nodeCapacityType, nodeInstanceType,
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    if (connection === null || !isCollectableAwsSourceKind(connection.sourceKind) || connection.status !== "active") throw Object.assign(new Error("Cloud connection not found"), { code: "NOT_FOUND", status: 404 });
    ...
    if (selected === null) return jsonResponse({ schema: "sutra.finops-kubecost-dashboard.v1", connectionId: connection.id, sourceState: "configuration_required", officialDefinition: KUBECOST_OFFICIAL_DEFINITION, dashboard: null,
      collection: { jobContractAvailable: true, providerAdapterAvailable: true, runtimeState: "unavailable",
        sharedWorkerRegistered: KUBECOST_PRODUCTION_COMPOSITION_STATUS.sharedWorkerRegistered,
        reason: KUBECOST_PRODUCTION_COMPOSITION_STATUS.activationState } });
    ...
    const runtimeState = sourceState === "complete" || sourceState === "empty" ? "ready"
      : sourceState === "failed" ? "failed"
        : sourceState === "configuration_required" ? "unavailable" : "collecting";
    return jsonResponse({ ...dashboard, connectionId: connection.id, sourceState, officialDefinition: KUBECOST_OFFICIAL_DEFINITION, history,
      ...
      collection: { jobContractAvailable: true, providerAdapterAvailable: true, runtimeState,
        sharedWorkerRegistered: KUBECOST_PRODUCTION_COMPOSITION_STATUS.sharedWorkerRegistered,
        reason: KUBECOST_PRODUCTION_COMPOSITION_STATUS.activationState },
